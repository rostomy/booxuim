...
class FilterUnvi extends StatefulWidget {
  final String title;
  final String domain;
  final String filiere;
  const FilterUnvi({Key key, this.title, this.domain, this.filiere})
      : super(key: key);
  @override
  _FilterUnviState createState() => _FilterUnviState();
}

class _FilterUnviState extends State<FilterUnvi> {
      ...
      body: BlocProvider(
        create: (context) => getIt<BookBloc>()
          ..add(BookEvent.getUnivBookss(
              ...
              "",
              "5f71eb3ba5319bbc6aeec242",
              widget.domain,
              widget.filiere)),
        child: BlocConsumer<BookBloc, BookState>(
          listener: (context, state) {
            state.getUnivBooksFailureOrSuccess.fold(() => null,
                (either) => either.fold((l) => print(l), (r) => null));
          },
          builder: (context, state) {
            return state.getUnivBooksFailureOrSuccess.fold(
                () => BookListShimmer(
                      length: 20,
                    ),
                (either) => either.fold(
